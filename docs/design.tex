...
\begin{tikzpicture}[node distance=1.5cm, font={\sf}, align=center]
	...
	%% status table
	\node (status-table) [resource, below=of monitor, label={[below]90: \textbf{Dynamodb}}] {Status table};
		\handshake{monitor}{status-table}{add new\\ items}{}{vertical}
...
Calculates and stores in Dynamodb the name that will be used to rename each incoming file at the SFTP server.
...
Each item in this table relates to a file uploaded to the Incoming bucket by MyInterview. Files created by function Extract audio will not be entered as a separate item in this table.
